...
| 扩展模块 | **CAS_to_Threshold** | 根据 CAS 号查询嗅觉阈值（精简库） |
...
已将《化合物嗅觉阈值汇编》第二版中物质在水中的嗅觉阈值整理成简易数据库，可通过CAS号快速查找物质对应的嗅觉阈值。
...
* 程序仅能对CAS号进行处理，不支持通过名称搜索，若只有名称，请先使用 CAS号查询 模块。
...
* 程序在搜索时会打开`Chrome浏览器`，请确保电脑中有该浏览器。
...
### 2026.07.28 v0.1.3
* 新增 [保留指数计算](#保留指数计算)、[含硅化合物标记](#含硅化合物标记) 功能（Si_Marker）
